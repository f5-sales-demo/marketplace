...
引擎提供本地自托管的网页抓取功能。无需 API 密钥、无需订阅、不依赖云服务。所有操作都在 devcontainer 内部的 `localhost:3002` 上运行的本地 firecrawl 实例上执行。
...
当您请求抓取 URL、爬取网站、映射站点 URL、搜索网络、提取结构化数据、生成 llms.txt、批量抓取多个 URL 或将网页转换为 markdown 时自动激活。会立即委派给 firecrawl-operator 代理。

## 代理

### firecrawl-operator

自主网页抓取代理，执行针对本地 firecrawl API 的 `curl` + `jq` 操作序列。支持覆盖所有 v1 端点的 11 种协议。只读代理（无写入、编辑或代理工具）。
...
当 `ENABLE_FIRECRAWL=true`（默认值）时，技术栈会自动启动。SessionStart 钩子会检查 API 是否可达，如果服务不可用则发出警告。
