...
description: 通过开源 Firecrawl 引擎实现本地自托管网页抓取与爬取。
sidebar:
  order: 13
i18n:
...
**f5xc-firecrawl** 插件通过开源 [firecrawl](https://github.com/mendableai/firecrawl) 引擎提供本地自托管网页抓取功能。无需 API 密钥，无需订阅，无需云端依赖。所有操作均在 devcontainer 内 `localhost:3002` 上的本地 firecrawl 实例中运行。
...
当您请求抓取 URL、爬取网站、映射站点 URL、搜索网络、提取结构化数据、生成 llms.txt、批量抓取多个 URL 或将网页转换为 Markdown 时自动激活。立即委托给 firecrawl-operator 代理。
...
当 `ENABLE_FIRECRAWL=true` 时（默认值），服务栈自动启动。SessionStart 钩子会检查 API 是否可达，并在服务不可用时发出警告。
