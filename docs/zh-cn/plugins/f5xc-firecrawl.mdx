...
description: 通过开源 Firecrawl 引擎实现本地自托管的网页抓取与爬取。
sidebar:
  order: 13
i18n:
...
**f5xc-firecrawl** 插件通过开源 [firecrawl](https://github.com/mendableai/firecrawl) 引擎提供本地自托管的网页抓取功能。无需 API 密钥、无需订阅、无需云端依赖。所有操作均在 devcontainer 内部 `localhost:3002` 上运行的本地 firecrawl 实例中执行。
...
当您请求抓取 URL、爬取网站、映射站点 URL、搜索网络、提取结构化数据、生成 llms.txt、批量抓取多个 URL 或将网页转换为 Markdown 时自动激活。立即委托给 firecrawl-operator 代理执行。

## 代理

### firecrawl-operator

自主网页抓取代理，针对本地 firecrawl API 执行 `curl` + `jq` 序列。支持覆盖所有 v1 端点的 11 种协议。只读代理（无写入、编辑或代理工具）。
...
该插件需要在 devcontainer 中运行 firecrawl 服务栈：
...
| LiteLLM proxy | OPENAI_BASE_URL | 用于提取的 LLM 后端（可选） |

当 `ENABLE_FIRECRAWL=true`（默认值）时，服务栈将自动启动。SessionStart 钩子会检查 API 是否可访问，并在服务不可用时发出警告。
