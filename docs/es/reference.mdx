...
| `metadata.description` | Sí | Descripción del marketplace legible por humanos |
...
Valide sus manifiestos de marketplace y plugin de forma local:
...
- Conformidad con el esquema de `marketplace.json`
- Que todos los directorios de plugins referenciados existen
- Que cada plugin tiene un `plugin.json` válido
- Que las habilidades tienen el frontmatter de `SKILL.md` con el formato correcto
- Que los comandos tienen los campos de frontmatter requeridos

## Configuración de equipo

### Preconfiguración de marketplaces
...
### Habilitación previa de plugins
...
<Aside type="tip">
  Confirmar estas configuraciones en su repositorio garantiza que cada
  miembro del equipo obtenga la misma configuración de marketplace y plugin
  de forma automática.
</Aside>
...
Si no aparece, agréguelo de nuevo:
...
- Confirme que el plugin está instalado:
  `/plugin list`
- Verifique que su versión de Claude Code admite plugins
  (v1.0.33+)
...
Las habilidades se activan en función del campo `description` en el frontmatter de SKILL.md. Si una habilidad no se activa cuando se esperaba:

- Verifique que la descripción describa con precisión el
...
las correcciones sugeridas de forma manual y luego vuelva a ejecutar la revisión para
