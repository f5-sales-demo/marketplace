...
description: 透過開源 Firecrawl 引擎實現本地自託管網頁抓取與爬取。
sidebar:
  order: 13
i18n:
...
**f5xc-firecrawl** 外掛程式透過開源 [firecrawl](https://github.com/mendableai/firecrawl) 引擎提供本地自託管網頁抓取功能。無需 API 金鑰、無需訂閱、無需雲端依賴。所有操作均在 devcontainer 內部 `localhost:3002` 上的本地 firecrawl 執行個體中執行。
...
<Aside type="note">
  extract 指令需要透過 `OPENAI_BASE_URL` 和 `OPENAI_API_KEY` 配置相容 OpenAI 的 LLM 代理。
</Aside>
...
當您要求抓取 URL、爬取網站、映射網站 URL、搜尋網路、擷取結構化資料、生成 llms.txt、批次抓取多個 URL 或將網頁轉換為 Markdown 時自動啟動。立即委派給 firecrawl-operator 代理程式。

## 代理程式

### firecrawl-operator

自主網頁抓取代理程式，針對本地 firecrawl API 執行 `curl` + `jq` 序列。支援涵蓋所有 v1 端點的 11 種協定。唯讀代理程式（無寫入、編輯或代理工具）。
...
此外掛程式需要在 devcontainer 中執行的 firecrawl 堆疊：

| 元件 | 連接埠 | 用途 |
|-----------|------|---------|
| Firecrawl API | 3002 | 所有抓取/爬取/映射/搜尋/擷取端點 |
| Playwright | 3000 | JavaScript 渲染引擎 |
| Redis | 6379 | 工作佇列後端 |
| PostgreSQL | socket | 爬取/批次工作持久化 |
| LiteLLM proxy | OPENAI_BASE_URL | extract 的 LLM 後端（選用） |

當 `ENABLE_FIRECRAWL=true`（預設值）時，堆疊會自動啟動。SessionStart 掛鉤會檢查 API 是否可連線，並在服務中斷時發出警告。
