...
操作都在 devcontainer 內的 `localhost:3002` 上執行本機 firecrawl 實例。
...
當您要求抓取 URL、爬取網站、映射網站 URL、搜尋網頁、擷取結構化資料、產生 llms.txt、
...
自主網頁抓取代理，針對本機 firecrawl API 執行 `curl` + `jq` 序列。
支援涵蓋所有 v1 端點的 11 種協定。唯讀代理（無 Write、Edit 或 Agent 工具）。
...
當 `ENABLE_FIRECRAWL=true`（預設值）時，堆疊會自動啟動。SessionStart 鉤子會檢查 API 是否可連線，
並在服務停止時發出警告。
