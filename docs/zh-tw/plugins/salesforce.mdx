---
title: salesforce
description: 使用 Salesforce CLI 進行組織驗證、帳戶管理、管道分析及自然語言資料查詢的自動化工具。
sidebar:
  label: Salesforce
  order: 14
i18n:
  sourceHash: d55112fac780
  translator: machine
---

import { Aside, Badge } from '@astrojs/starlight/components';

**salesforce** 外掛程式將 Salesforce CLI 與 Claude Code 整合，
提供組織驗證及通用 CLI 代理，可執行任何 `sf` 指令，包括 SOQL 查詢。它支援使用 JWT、存取權杖及 SFDX URL 驗證流程的無頭容器環境。以下使用指南示範如何透過 CLI 代理以自然語言查詢帳戶、商機、案例及聯絡人。

<Badge text="v1.0.0" variant="note" /> <Badge text="Development" variant="tip" />

## 安裝

```
/plugin install salesforce@f5xc-salesdemos-marketplace
```

## 先決條件

- **Salesforce CLI**（`@salesforce/cli`）
- 具有 API 存取權限的 **Salesforce 組織**
- 具有查詢物件（案例、商機、帳戶、聯絡人）權限的使用者帳戶

在您的工作站上安裝 Salesforce CLI：

```bash
brew install sf
```

驗證安裝結果：

```bash
sf --version
```

您應該會看到類似 `@salesforce/cli/2.x.x` 的輸出。

## 驗證設定

### 步驟一：找到您的 Salesforce 網域

查看瀏覽器中的 Salesforce 網址。若您透過
`https://acme.lightning.force.com` 存取 Salesforce，則您的登入網域為
`https://acme.my.salesforce.com`。

<Aside type="tip">
  若您的公司使用 SSO，您必須透過 `--instance-url` 指定自訂網域，
  以便登入流程透過您的 SSO 提供者進行重新導向，
  而非使用一般的 Salesforce 登入頁面。
</Aside>

### 步驟二：透過瀏覽器進行驗證

```bash
sf org login web --alias my-org --set-default --instance-url https://YOUR-DOMAIN.my.salesforce.com
```

將 `YOUR-DOMAIN` 替換為步驟一中的公司網域。您的瀏覽器將開啟
— 完成 SSO 或登入流程並授權應用程式。您應該會看到：

```
Successfully authorized your-email@company.com with org ID 00DXXXXXXXXXXXXXXX
```

### 步驟三：驗證連線

```bash
sf org display --target-org my-org
```

確認輸出顯示 `Connected Status: Connected`，以及您的使用者名稱
和執行個體 URL。

### 步驟四：容器的可攜式驗證（選用）

若您需要在無頭環境（CI/CD、容器、遠端工作階段）中進行驗證，
請從您的工作站匯出 SFDX 驗證 URL：

```bash
sf org display --verbose --target-org my-org
```

複製 **Sfdx Auth Url** 值（以 `force://` 開頭）。在容器中執行：

```bash
export SFDX_AUTH_URL="force://PlatformCLI::YOUR_AUTH_TOKEN@your-domain.my.salesforce.com"
echo "$SFDX_AUTH_URL" | sf org login sfdx-url --sfdx-url-stdin=- --alias=my-org --set-default
```

<Aside type="caution">
  SFDX 驗證 URL 是一項憑證——請像對待密碼一樣謹慎處理。請勿
  將其提交至檔案或貼入聊天訊息中。僅使用環境變數
  和 stdin 管道。
</Aside>

### 驗證方法參考

| 方法 | 最適合 | 需求 |
|--------|----------|----------|
| 網頁登入 | 具有瀏覽器的工作站 | 瀏覽器 + SSO |
| SFDX URL | 容器、CI/CD、可攜式驗證 | 來自已驗證工作階段的驗證 URL |
| JWT Bearer | 自動化管道 | 已連線應用程式 + 私密金鑰 + 消費者金鑰 |
| 存取權杖 | 環境變數驗證 | `SF_ACCESS_TOKEN` + `SF_ORG_INSTANCE_URL` |

**注意：** 裝置流程（`sf org login device`）自 2025 年 8 月起已封鎖，
不再受支援。

### 環境變數

| 變數 | 用途 |
|----------|---------|
| `SF_ACCESS_TOKEN` | 用於存取權杖驗證的 Bearer 權杖 |
| `SFDX_AUTH_URL` | 用於 SFDX URL 驗證的 Force 驗證 URL |
| `SF_ORG_INSTANCE_URL` | 組織執行個體 URL |
| `SF_JWT_KEY_FILE` | JWT 私密金鑰的路徑 |
| `SF_CLIENT_ID` | 已連線應用程式消費者金鑰 |
| `SF_USERNAME` | 用於 JWT 的 Salesforce 使用者名稱 |

## 指令

### /salesforce:sf-status

檢查您的 Salesforce 組織連線狀態、CLI 版本及已驗證的組織。

```
/salesforce:sf-status
```

**預期結果：** 顯示 sf CLI 版本、已驗證的組織別名、使用者名稱、執行個體 URL、連線狀態及 API 版本的表格。

### /salesforce:sf-login

驗證至 Salesforce 組織。可選擇性地提供別名。

```
/salesforce:sf-login
/salesforce:sf-login my-org
```

**預期結果：** 若具有該別名的組織已完成驗證，則回報連線詳細資訊。若未驗證，則檢查可用的憑證（環境變數），並使用第一個完全滿足條件的方法進行驗證。

## 技能

### salesforce-index

頂層意圖路由器。當您以自然語言提及 Salesforce、組織管理或資料查詢時自動啟動。

| 您說的內容 | 路由至 |
|---------|-----------|
| 「登入 salesforce」、「驗證組織」 | `salesforce-auth` 技能 |
| 「檢查組織狀態」、「列出組織」 | `cli-operator` 代理 |
| 「顯示我的商機」、「查詢案例」 | `cli-operator` 代理（SOQL 查詢） |
| 「產生 Apex 類別」、「建立 Flow」 | afv-library 開發技能 |

### salesforce-auth

適用於容器的驗證技能。支援四種驗證方法，並自動偵測可用的憑證。優先順序為：
存取權杖、JWT、SFDX URL，最後才是網頁登入。

## 代理

### cli-operator

具有安全防護措施的自主 Salesforce CLI 代理，執行 `sf` 指令。所有 Salesforce 操作均委派給此代理，以保持主工作階段的精簡。

**安全規則：**

- 預設為唯讀——寫入操作需要明確核准
- 永不在未預覽及確認的情況下進行部署
- 永不在輸出中顯示存取權杖、驗證 URL 或憑證
- 清理所有使用者提供的值（拒絕 shell 特殊字元）
- 使用 `--json` 輸出以取得結構化、可解析的結果

**回應格式：**

```
## Result: [SUCCESS | FAILURE | PARTIAL]
### Command Executed
### Output Summary
### Issues
```

## 使用指南：帳戶管理

<Aside type="note">
  以下提示並非外掛程式的硬式編碼功能。它們能夠運作是因為
  cli-operator 代理可以執行任何 `sf` CLI 指令，包括任意 SOQL 查詢。
  結果取決於您組織的資料模型及您的使用者權限。此處的提示已
  使用標準 Salesforce 物件（Account、Contact、Case、Opportunity、
  AccountTeamMember、OpportunityTeamMember）進行測試並驗證可正常運作。
</Aside>

這些提示適用於任何 Salesforce 組織。請將預留位置值替換為您自己的資訊。每個提示產生的結果均特定於您的角色、帳戶及區域。

<Aside type="tip">
  為獲得最佳結果，請在需要識別您的使用者記錄的提示中包含您的電子郵件地址。
  Salesforce 組織中可能有多個姓名相似的使用者。
</Aside>

### 開始使用

驗證後，確認一切正常運作：

```
/salesforce:sf-status
```

然後嘗試簡單的自然語言查詢：

```
list my authenticated salesforce orgs
```

**預期結果：** 顯示所有已連線組織的表格，包含別名、使用者名稱、執行個體 URL 及連線狀態。

### 尋找您的帳戶

探索您被指派的帳戶：

```
what salesforce accounts am I on the account team for? My email is your-email@company.com
```

**預期結果：** 依帳戶擁有者分組的帳戶清單，顯示您在每個帳戶中的團隊成員角色。若大多數角色顯示為空白，這是常見的資料品質問題——帳戶團隊成員身份存在，但角色未填入。

### 與同事交叉比對覆蓋範圍

與隊友比較帳戶覆蓋範圍以找出缺口：

```
In salesforce, find all accounts where Colleague Name is on the account team. Then for each of those accounts, check if your-email@company.com is also on the account team. Show me two lists: accounts where we are BOTH tagged, and accounts where only my colleague is tagged but I am missing.
```

**預期結果：** 顯示重疊數量和缺口數量的摘要表格，隨後是兩個清單。若您最近更換了角色，您可能發現零重疊——這揭示了哪些帳戶需要將您加入。

<Aside type="note">
  您也可以透過新增以下內容來檢查前任人員：「也檢查
  前任人員姓名 是否出現在這些帳戶中。」這有助於
  在接替某個區域的人員時識別交接缺口。
</Aside>

### 區域管道概覽

查看您帳戶中所有未結商機：

```
show me all open salesforce opportunities on Colleague Name's account team accounts, sorted by close date, include the stage, amount, and probability
```

**預期結果：** 包含商機總數、原始管道及加權管道的摘要。然後是階段分佈表格，以及按金額排列的頂級商機清單。輸出會標記資料品質問題，例如已過期的結束日期或空值金額。

### 商機深入分析

取得特定交易的完整資訊：

```
show me a detailed view of the OPPORTUNITY NAME opportunity in salesforce - include the opportunity team members, any activities or tasks, and the account contacts
```

**預期結果：** 商機概覽（階段、金額、結束日期、機率、預測類別、擁有者）、具有角色的商機團隊成員、標記的聯絡人、近期任務和事件，以及顯示交易金額和結束日期隨時間變化的欄位變更歷史記錄。

### 整個區域的支援案例

檢查您帳戶中的未結支援案例：

```
show me all open salesforce cases across Colleague Name's account team accounts, grouped by account, sorted by most recent first
```

**預期結果：** 包含優先順序和狀態細分的未結案例數量，然後是依帳戶分組的案例詳細資訊。長時間未有活動的陳舊案例（數月或數年）會被標記為資料品質候選項目。

### 季度管道預測

取得適合預測的管道視圖：

```
for Colleague Name's accounts, show me a quarterly pipeline summary - group the open opportunities by close date quarter with count, total amount, and weighted amount for each quarter
```

**預期結果：** 每季一列的表格，顯示商機數量、管道總計及加權管道。包含每季的階段組合細分，並突顯加權最高的交易。已過期的商機會單獨分組。

### 完整帳戶概覽

深入了解特定客戶：

```
give me a full account overview for ACCOUNT NAME in salesforce including contacts, open opportunities, and recent cases
```

**預期結果：** 公司概況（產業、營收、員工人數、地點）、具有職稱和電子郵件的關鍵聯絡人、具有階段和金額的未結商機，以及具有狀態的近期支援案例。

### 案例查詢

依編號查詢特定支援案例：

```
look up salesforce case CASE-NUMBER and show me the case details, the customer account, and who owns it
```

**預期結果：** 案例主旨、狀態、優先順序、描述、客戶帳戶概況，以及具有聯絡資訊的案例擁有者。

## 使用指南：資料查詢

進階使用者可以以自然語言要求特定的 SOQL 式查詢。外掛程式會將您的請求轉換為適當的 SOQL 並執行。

### 特定欄位

```
query salesforce for all Contacts at ACCOUNT NAME - show Name, Title, Email, Phone, and Department
```

### 日期篩選

```
show me all salesforce opportunities that closed won in the last 90 days on ACCOUNT NAME
```

### 聚合

```
count all open salesforce cases grouped by priority and status across my accounts
```

### 自訂物件

```
query the CUSTOM_OBJECT__c object in salesforce for records where Status__c = 'Active'
```

<Aside type="tip">
  若查詢因欄位不存在而失敗，外掛程式會自動重試並排除該欄位。
  您組織中的自訂欄位可能使用不同的名稱——若找不到欄位，
  請檢查您的 Salesforce 設定。
</Aside>

## 開發技能

來自 [forcedotcom/afv-library](https://github.com/forcedotcom/afv-library) 的 30 個 Salesforce 開發技能會針對 Apex、Flow、LWC、SOQL、中繼資料、Agentforce 及部署任務自動啟動。請單獨安裝：

```bash
npx skills add forcedotcom/afv-library
```

| 主題 | 技能 |
|-------|-------|
| Apex 類別和服務 | `generating-apex` |
| Apex 測試 | `generating-apex-test` |
| Flow | `generating-flow` |
| LWC 和 UI 套件組合 | `building-ui-bundle-app` |
| 自訂物件 | `generating-custom-object` |
| 自訂欄位 | `generating-custom-field` |
| 驗證規則 | `generating-validation-rule` |
| 權限集 | `generating-permission-set` |
| FlexiPages | `generating-flexipage` |
| Agentforce 代理 | `developing-agentforce` |
| Agentforce 測試 | `testing-agentforce` |
| 部署 | `deploying-ui-bundle` |
| SLDS2 遷移 | `uplifting-components-to-slds2` |
| 觸發器重構 | `trigger-refactor-pipeline` |
