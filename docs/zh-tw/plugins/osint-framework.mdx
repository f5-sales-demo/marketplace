...
description: 'OSINT 工具目錄與調查技能，涵蓋 34 個類別共 1,064 個情報收集工具。'
sidebar:
  order: 12
i18n:
...
**osint-framework** 插件提供 OSINT 工具目錄與調查技能——來自 [osintframework.com](https://osintframework.com) 的 1,064 個免費情報收集工具，涵蓋 34 個類別。包含基於類別的技能、可執行的調查流程、CLI 工具執行、具備操作安全意識的工作流程，以及用於跨調查關聯的持久實體圖譜。
...
依類別瀏覽完整工具目錄。不帶參數時顯示所有 34 個類別及其工具數量。
...
頂層意圖路由器。當您說「查找相關資訊」、「調查」、「查詢」、「背景調查」，或描述任何開源情報任務時自動啟動。根據目標類型路由至正確的類別技能。
...
每個類別均有專屬技能，包含 CLI 工具、網路資源、調查工作流程、跨類別樞紐分析及操作安全注意事項。
...
<Aside type="tip">
  使用 `/osint-catalog` 瀏覽全部 34 個類別，或使用
  `/osint-search <能力>` 尋找特定任務的工具。
</Aside>

### osint-catalog

工具目錄瀏覽器。提供完整工具註冊表、類別索引、調查流程、速率限制參考資料及關聯引擎說明文件的存取功能。

## 代理程式

### osint-investigator

自主多工具調查協調器。執行完整調查工作流程——工具可用性檢查、目標類型偵測、循序工具執行（優先被動式）、結果彙整，以及附帶信心水準與操作安全分類的結構化報告生成。

與實體圖譜整合：每個工具結果均會在 `/tmp/osint-graph/` 中建立實體與關係，以供跨調查關聯使用。

<Aside type="caution">
  主動偵查工具（nmap、nuclei、sherlock）會直接與目標基礎設施接觸。操作安全警告鉤子會在執行主動工具前發出提示。請務必確認您已獲得授權。
</Aside>

### osint-researcher

唯讀目錄查詢代理程式。搜尋參考文件以推薦最適合指定任務的工具。僅作推薦與說明，從不執行工具。由類別技能在工具探索時內部呼叫使用。

### osint-executor

單一工具 CLI 執行代理程式。每次執行一個工具並帶入特定參數，解析輸出並回傳結構化結果。包含 20 多個工具的逐工具執行配方，含精確旗標、輸出格式及 jq 解析模式。
