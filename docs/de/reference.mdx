---
title: Referenz
description: >-
  marketplace.json-Schema-Referenz mit Felddefinitionen, Skill-Konfiguration und
  Validierungsregeln.
sidebar:
  order: 7
i18n:
  sourceHash: 15be095fe190
  translator: machine
---

import { Aside } from '@astrojs/starlight/components';

## marketplace.json-Schema

Das Marketplace-Manifest befindet sich unter
`.claude-plugin/marketplace.json` im Repository-Stammverzeichnis.

### Felder der obersten Ebene

| Feld | Erforderlich | Beschreibung |
|-------|----------|-------------|
| `$schema` | Nein | JSON-Schema-URL zur Validierung |
| `name` | Ja | Eindeutiger Marketplace-Bezeichner |
| `version` | Nein | Marketplace-Schema-Version (Semver) |
| `metadata.description` | Ja | Menschenlesbare Marketplace-Beschreibung |
| `owner.name` | Ja | Organisation oder Benutzer, der den Marketplace besitzt |
| `owner.url` | Nein | URL zum Profil oder zur Website des Eigentümers |
| `owner.email` | Nein | Kontakt-E-Mail |
| `plugins` | Ja | Array von Plugin-Einträgen |

### Felder eines Plugin-Eintrags

| Feld | Erforderlich | Beschreibung |
|-------|----------|-------------|
| `name` | Ja | Plugin-Bezeichner (muss mit `plugin.json` übereinstimmen) |
| `description` | Ja | Kurzbeschreibung des Plugins |
| `version` | Ja | Semantische Versionszeichenkette |
| `author.name` | Ja | Plugin-Autor |
| `source` | Ja | Pfad oder URL zum Plugin (siehe Quelltypen) |
| `category` | Ja | Plugin-Kategorie (z. B. `productivity`) |
| `homepage` | Nein | URL zur Plugin-Dokumentation oder Homepage |
| `license` | Nein | SPDX-Lizenzbezeichner |
| `keywords` | Nein | Array von Suchbegriffen |
| `tags` | Nein | Array von Tags zur Filterung |
| `repository` | Nein | Repository-URL |

## Plugin-Quelltypen

Das Feld `source` in marketplace.json unterstützt mehrere
Formate:

| Typ | Beispiel | Beschreibung |
|------|---------|-------------|
| Relativer Pfad | `./plugins/my-plugin` | Plugin im selben Repository |
| GitHub-Kurzform | `owner/repo` | Plugin im Repository-Stammverzeichnis |
| GitHub mit Pfad | `owner/repo/path/to/plugin` | Plugin in einem Unterverzeichnis |
| Git-URL | `https://github.com/owner/repo.git` | Git-Repository |
| npm-Paket | `npm:@scope/package` | Veröffentlichtes npm-Paket |

## plugin.json-Schema

Jedes Plugin hat ein Manifest unter
`.claude-plugin/plugin.json` in seinem Verzeichnis.

| Feld | Erforderlich | Beschreibung |
|-------|----------|-------------|
| `name` | Ja | Plugin-Bezeichner |
| `description` | Ja | Was das Plugin macht |
| `version` | Ja | Semantische Version |
| `author.name` | Ja | Plugin-Autor |
| `author.url` | Nein | URL zum Profil oder zur Website des Autors |
| `homepage` | Nein | Dokumentations-URL |
| `keywords` | Nein | Suchbegriffe |
| `license` | Nein | SPDX-Lizenzbezeichner |
| `repository` | Nein | Repository-URL |

## Plugin-Verzeichnisstruktur

```
plugins/my-plugin/
  .claude-plugin/
    plugin.json          # Plugin-Manifest (erforderlich)
  skills/
    skill-name/
      SKILL.md           # Skill-Definition (erforderlich pro Skill)
      references/        # Referenzdatendateien (optional)
  commands/
    command-name.md      # Befehlsdefinition (eine pro Befehl)
  agents/
    agent-name.md        # Agentendefinition (eine pro Agent)
  README.md              # Plugin-Dokumentation (empfohlen)
```

## Umgebungsvariablen

Plugins können diese Variablen in ihren Skill- und
Befehlsdateien verwenden:

| Variable | Beschreibung |
|----------|-------------|
| `$\{CLAUDE_PLUGIN_ROOT\}` | Absoluter Pfad zum Stammverzeichnis des Plugins |

## Validierung

Validieren Sie Ihre Marketplace- und Plugin-Manifeste lokal:

```bash
claude plugin validate .
```

Dies prüft:

- `marketplace.json`-Schema-Konformität
- Alle referenzierten Plugin-Verzeichnisse existieren
- Jedes Plugin hat eine gültige `plugin.json`
- Skills haben korrekt formatiertes `SKILL.md`-Frontmatter
- Befehle haben die erforderlichen Frontmatter-Felder

## Team-Konfiguration

### Marketplaces vorkonfigurieren

Fügen Sie dies zur `.claude/settings.json` in einem beliebigen Repository hinzu:

```json
{
  "extraKnownMarketplaces": [
    "f5xc-salesdemos/marketplace"
  ]
}
```

### Plugins voraktivieren

```json
{
  "enabledPlugins": [
    "f5xc-docs-tools@f5xc-salesdemos-marketplace"
  ]
}
```

<Aside type="tip">
  Das Einchecken dieser Einstellungen in Ihr Repository stellt sicher, dass
  jedes Teammitglied automatisch die gleiche Marketplace- und
  Plugin-Konfiguration erhält.
</Aside>

## Fehlerbehebung

### Marketplace nicht gefunden

Überprüfen Sie, ob der Marketplace hinzugefügt wurde:

```
/plugin marketplace list
```

Falls nicht vorhanden, fügen Sie ihn erneut hinzu:

```
/plugin marketplace add f5xc-salesdemos/marketplace
```

### Plugin wird nicht aktiviert

- Bestätigen Sie, dass das Plugin installiert ist:
  `/plugin list`
- Prüfen Sie, ob Ihre Claude Code-Version Plugins unterstützt
  (v1.0.33+)
- Überprüfen Sie, ob die Auslösebedingungen des Skills mit Ihrem
  aktuellen Kontext übereinstimmen

### Skills werden nicht automatisch ausgelöst

Skills werden basierend auf ihrem `description`-Feld im
SKILL.md-Frontmatter aktiviert. Wenn ein Skill nicht wie
erwartet aktiviert wird:

- Prüfen Sie, ob die Beschreibung den Auslösekontext
  genau beschreibt
- Verwenden Sie den Befehl direkt (z. B. `/f5xc-docs-tools:review-mdx`)
  als Behelfslösung

### Build-Fehler nach der Überprüfung

Das Plugin meldet Probleme, behebt sie jedoch nicht automatisch. Wenden Sie
die vorgeschlagenen Korrekturen manuell an und führen Sie dann die Überprüfung
erneut aus, um die Behebung zu bestätigen.
