...
| `metadata.description` | Ja | Menschenlesbare Marktplatz-Beschreibung |
| `owner.name` | Ja | Organisation oder Benutzer, der den Marktplatz besitzt |
...
| `description` | Ja | Was das Plugin tut |
...
plugins/example-plugin/
  ...
  skills/
    skill-name/
      SKILL.md           # Skill-Definition (erforderlich pro Skill)
...
<Aside type="tip">
  Wenn Sie diese Einstellungen in Ihr Repository übertragen, stellen Sie sicher, dass jedes
  Teammitglied automatisch dieselbe Marktplatz- und Plugin-Konfiguration erhält.
</Aside>
...
- Bestätigen Sie, dass das Plugin installiert ist:
  `/plugin list`
- Überprüfen Sie, ob Ihre Claude Code-Version Plugins unterstützt
  (v1.0.33+)
- Stellen Sie sicher, dass die Auslösebedingungen des Skills mit Ihrem aktuellen
...
- Überprüfen Sie, ob die Beschreibung den Auslösekontext genau beschreibt
- Verwenden Sie den Befehl direkt (z. B. `/f5xc-docs-tools:review-mdx`)
  als Workaround

### Build-Fehler nach der Überprüfung

Das Plugin meldet Probleme, behebt diese jedoch nicht automatisch. Wenden Sie
