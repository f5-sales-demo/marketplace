...
| `homepage` | Não | URL para documentação ou página inicial do plugin |
...
| Abreviação GitHub | `owner/repo` | Plugin na raiz do repositório |
...
Os plugins podem usar estas variáveis em seus arquivos de
skill e comando:
...
- Conformidade do `marketplace.json` com o esquema
- Todos os diretórios de plugin referenciados existem
- Cada plugin possui um `plugin.json` válido
- Skills possuem frontmatter `SKILL.md` formatado corretamente
- Comandos possuem campos de frontmatter obrigatórios
...
<Aside type="tip">
  ...
  membro da equipe receba a mesma configuração de marketplace e plugins
  automaticamente.
</Aside>
...
as correções sugeridas manualmente e, em seguida, execute novamente a revisão para
