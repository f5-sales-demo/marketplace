...
description: >-
  Raspagem e rastreamento web local auto-hospedado via o motor de código aberto
  Firecrawl.
sidebar:
  order: 13
i18n:
...
via o motor de código aberto [firecrawl](https://github.com/mendableai/firecrawl).
...
Extração de dados estruturados de páginas web com tecnologia LLM.
...
Ativa automaticamente quando você solicita raspar uma URL, rastrear um site,
mapear URLs de um site, pesquisar na web, extrair dados estruturados, gerar llms.txt,
raspar múltiplas URLs em lote ou converter uma página web em markdown.
...
Este plugin utiliza a versão de código aberto auto-hospedada:

- Não requer autenticação ou chaves de API para raspagem
