...
| `tags` | Non | Tableau de balises pour le filtrage |
...
| `author.name` | Oui | Auteur du plugin |
| `author.url` | Non | URL vers le profil de l'auteur ou son site web |
...
Validez vos manifestes de place de marché et de plugin localement :
...
- Le formatage correct du frontmatter `SKILL.md` pour les compétences
- La présence des champs de frontmatter requis dans les commandes
...
### Activation préalable des plugins
...
<Aside type="tip">
  L'enregistrement de ces paramètres dans votre dépôt garantit que chaque
...
### Compétences qui ne se déclenchent pas automatiquement

Les compétences s'activent en fonction de leur champ `description` dans
le frontmatter SKILL.md. Si une compétence ne s'active pas comme prévu :
...
### Erreurs de build après la révision
