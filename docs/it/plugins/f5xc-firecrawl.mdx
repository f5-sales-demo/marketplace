...
Esegue lo scraping di un singolo URL ed estrae il contenuto in markdown.
...
Esegue ricerche sul web con la possibilità di fare scraping dei risultati.
...
Si attiva automaticamente quando si richiede di eseguire lo scraping di un URL, il crawling di un sito web, la mappatura degli URL di un sito, la ricerca sul web, l'estrazione di dati strutturati, la generazione di llms.txt, lo scraping batch di più URL, o la conversione di una pagina web in markdown.
...
sull'API firecrawl locale. Supporta 11 protocolli che coprono tutti
gli endpoint v1. Agente di sola lettura (nessuno strumento Write, Edit o Agent).
...
Il plugin richiede lo stack firecrawl in esecuzione nel contenitore di sviluppo:
...
- Nessun limite di crediti o limitazione della velocità
- Utilizza gli endpoint API v1 (non v2)
- Sessioni browser e ricerca approfondita non disponibili
- Extract utilizza il proprio proxy LLM anziché i modelli hosted
- Eseguito interamente all'interno della rete del container locale
