...
이 가이드는 f5-sales-demo 마켓플레이스를 Claude Code에 추가하고
첫 번째 플러그인을 설치하는 과정을 안내합니다.

## 사전 요구 사항

- Claude Code **v1.0.33** 이상 (플러그인 마켓플레이스 지원)
- [f5-sales-demo](https://github.com/f5-sales-demo) 조직에 접근 권한이 있는 GitHub 계정
...
1. **마켓플레이스 추가**
   ...
   이 명령은 마켓플레이스를 플러그인 소스로 등록합니다. 이 작업은
   한 번만 수행하면 됩니다.

2. **사용 가능한 플러그인 탐색**

   마켓플레이스에서 제공하는 항목을 나열합니다:
...
4. **플러그인 사용**
   ...
   임의의 f5-sales-demo 콘텐츠 저장소로 이동합니다.

   **docs tools**의 경우, MDX 검토 명령을 실행합니다:

   ```
   /f5xc-docs-tools:review-mdx
   ```

   **sales engineer**의 경우, "prepare the demo", "run the demo",
   "walk through the demo"와 같은 트리거 문구를 사용하면
   플러그인이 자동으로 활성화됩니다.
...
팀 전체를 위해 마켓플레이스를 미리 구성하려면, 프로젝트의
...
<Aside type="tip" title="공유 설정">
  `.claude/settings.json`을 저장소에 커밋하면 모든 팀 구성원이
  마켓플레이스와 플러그인을 자동으로 사용할 수 있게 됩니다 —
  별도의 수동 설정이 필요하지 않습니다.
</Aside>

이를 통해 모든 기여자가 저장소에서 첫 번째 Claude Code 세션부터
동일한 도구를 사용할 수 있습니다.
