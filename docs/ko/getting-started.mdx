...
이 가이드는 Claude Code에 f5xc-salesdemos
마켓플레이스를 추가하고 첫 번째 플러그인을 설치하는 과정을 안내합니다.
...
4. **플러그인 사용**
   ...
   **sales engineer**의 경우, "prepare the demo", "run the demo" 또는 "walk through
   the demo"와 같은 트리거 문구를 사용합니다. 플러그인이 자동으로 활성화됩니다.
...
<Aside type="tip" title="공유 설정">
  `.claude/settings.json`을 리포지토리에 커밋하면
  모든 팀원이 마켓플레이스와 플러그인을
  자동으로 사용할 수 있습니다 — 수동 설정이 필요하지 않습니다.
</Aside>

이를 통해 모든 기여자가 리포지토리에서 첫 번째 Claude Code
세션부터 동일한 도구를 사용할 수 있습니다.
