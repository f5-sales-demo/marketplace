---
title: 참조
description: '필드 정의, 스킬 구성 및 검증 규칙이 포함된 marketplace.json 스키마 참조.'
sidebar:
  order: 7
i18n:
...
| 필드 | 필수 여부 | 설명 |
|-------|----------|-------------|
| `$schema` | 아니오 | 검증을 위한 JSON 스키마 URL |
...
| GitHub 약식 | `owner/repo` | 저장소 루트의 플러그인 |
...
각 플러그인은 해당 디렉터리 내의
`.claude-plugin/plugin.json`에 매니페스트를 보유합니다.
...
- 스킬의 올바른 형식의 `SKILL.md` 프론트매터 여부
- 명령의 필수 프론트매터 필드 포함 여부

## 팀 구성

### 마켓플레이스 사전 구성

모든 저장소의 `.claude/settings.json`에 추가합니다:
...
<Aside type="tip">
  이 설정을 저장소에 커밋하면 모든 팀원이 동일한
  마켓플레이스 및 플러그인 구성을 자동으로
  적용받을 수 있습니다.
</Aside>
...
활성화됩니다. 예상한 경우에 스킬이 활성화되지 않는다면:

- 설명이 트리거 컨텍스트를 정확하게 설명하는지 확인합니다
- 해결 방법으로 명령을 직접 사용합니다 (예: `/f5xc-docs-tools:review-mdx`)
