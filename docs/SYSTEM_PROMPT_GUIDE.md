...
- **Standard**: Pair every guideline, constraint, or workflow step with an explicit explanation of *why* the rule exists and what benefit it provides.
- **Rationale**: Providing rationale enables Claude models to generalize safety rules intelligently when encountering novel, unscripted edge cases, rather than applying rules blindly or rigidly.
