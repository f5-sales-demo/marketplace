...
test('automatically retires an owned v1 ingress marker before applying its corrected successor', async () => {
  const f = fixture();
  ...
  expect((f.values.get('aws-platform-ingress.json') as { schemaVersion: number }).schemaVersion).toBe(2);
});
