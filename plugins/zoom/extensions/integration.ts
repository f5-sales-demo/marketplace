...
const publicXorgCall = (command: string, action: string | undefined, params: Record<string, unknown>) => {
  const result = Bun.spawnSync(["xorgctl", "--json", command, ...(action ? [action] : []), "--params", JSON.stringify(params)]);
  return { exitCode: result.exitCode, output: redact(new TextDecoder().decode(result.stdout)), error: redact(new TextDecoder().decode(result.stderr)) };
};
/** Zoom owns semantics; Xorg receives only its documented generic JSON calls. */
export const call = (action: Action, args: string[]) => {
  if (action === "join") {
    const target = args.join(" ");
    if (isInvitation(target)) return { exitCode: 1, output: JSON.stringify({ ok: false, error: "invitation_url_requires_stdin" }), error: "" };
  ...
  const shortcuts: Record<string, string> = { leave: "ALT+Q", "stop-share": "ALT+SHIFT+S", audio: "ALT+A", video: "ALT+V", share: "ALT+SHIFT+S" };
...
export default function zoomIntegration(pi: ExtensionApi) {
  ...
  pi.tools?.register({ name: "zoom_meeting", description: "Join or control Zoom through verified public xorgctl JSON.", inputSchema: { type: "object", properties: { command: { type: "string" } }, required: ["command"] }, async execute(input: { command: string }) { const parsed = parseZoomCommand(input.command); return call(parsed.action, parsed.args); } });
}
