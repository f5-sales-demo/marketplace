...
test_kvm_marketplace_entry() {
  ...
  if [ -f "$marketplace_manifest" ]; then
    jq -e '[.plugins[] | select(.name == "kvm" and .version == "3.0.4" and .source == "./plugins/kvm" and .recommended == true and .lifecycle.setupAuthorization == "install")] | length == 1' "$marketplace_manifest" >/dev/null
  else
    jq -e '.name == "kvm" and .version == "3.0.4"' "$PLUGIN_ROOT/.xcsh-plugin/plugin.json" >/dev/null
  fi
}
