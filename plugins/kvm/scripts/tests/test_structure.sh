...
test_kvm_marketplace_entry() {
  ...
  if [ -f "$marketplace_manifest" ]; then
    jq -e '[.plugins[] | select(.name == "kvm" and .version == "2.0.2" and .source == "./plugins/kvm" and .recommended == true and .lifecycle.setupAuthorization == "install")] | length == 1' "$marketplace_manifest" >/dev/null
  else
    jq -e '.name == "kvm" and .version == "2.0.2"' "$PLUGIN_ROOT/.xcsh-plugin/plugin.json" >/dev/null
  fi
}

test_kvm_required_surfaces() {
  local path
  for path in package.json bun.lock tsconfig.json artifacts.json README.md knowledge/ledger.json \
    src/index.ts src/tools.ts scripts/kvm-smsv2ctl scripts/kvm_smsv2_controller.py \
    terraform/versions.tf terraform/main.tf terraform/variables.tf terraform/outputs.tf \
    terraform/.terraform.lock.hcl terraform/ensure-image.sh terraform/wait-registration.py \
...
test_kvm_installed_cache_executes() {
  (
    ...
    jq -e '.schemaVersion == "kvm.smsv2/v2" and .controllerVersion == "2.0.0" and .action == "readiness" and .ok == true and .result.checks.artifacts == true' \
