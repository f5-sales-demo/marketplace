...
test_kvm_marketplace_entry() {
  ...
  if [ -f "$marketplace_manifest" ]; then
    jq -e '[.plugins[] | select(.name == "kvm" and .version == "3.0.1" and .source == "./plugins/kvm" and .recommended == true and .lifecycle.setupAuthorization == "install")] | length == 1' "$marketplace_manifest" >/dev/null
  else
    jq -e '.name == "kvm" and .version == "3.0.1"' "$PLUGIN_ROOT/.xcsh-plugin/plugin.json" >/dev/null
  fi
}
...
test_kvm_installed_cache_executes() {
  (
    ...
    jq -e '.schemaVersion == "kvm.smsv2/v3" and .controllerVersion == "3.0.1" and .action == "readiness" and .ok == true and .result.checks.artifacts == true' \
