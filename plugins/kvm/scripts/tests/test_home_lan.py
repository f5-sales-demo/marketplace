...
class HomeLanContracts(unittest.TestCase):
    def test_selected_application_namespace_persists_and_cannot_be_changed(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with mock.patch.object(controller, "storage_root", return_value="/data/libvirt/images"):
                config = controller._config(store, {
                    "siteName": "onprem-nuc-kvm",
                    "applicationNamespace": "multi-cloud-networking",
                })
                self.assertEqual(config["namespace"], "system")
                self.assertEqual(config["applicationNamespace"], "multi-cloud-networking")
                self.assertEqual(controller._config(store, {})["applicationNamespace"], "multi-cloud-networking")
                with self.assertRaisesRegex(controller.ControllerError, "persisted application namespace"):
                    controller._config(store, {"applicationNamespace": "another-project"})
            with self.assertRaisesRegex(controller.ControllerError, "invalid or system"):
                controller._config(controller.StateStore(pathlib.Path(directory) / "new"), {
    ...
    def test_apply_plan_requires_one_vip_and_one_origin(self):
        def change(address):
            return {
                ...
                "change": {"actions": ["create"], "after": {"namespace": "multi-cloud-networking"}},
            }
        ...
        for changes in (
            ...
            with self.assertRaises(controller.ControllerError):
                controller.require_home_lan_plan({"resource_changes": changes}, "multi-cloud-networking")
        wrong = {"resource_changes": [
            {**change("xcsh_http_loadbalancer.home"), "change": {"actions": ["create"], "after": {"namespace": "system"}}},
            change("xcsh_origin_pool.home"),
        ]}
    ...
    def test_exact_application_ownership_rejects_label_or_name_drift(self):
        ...
        with mock.patch.object(controller, "_xc_json", return_value=owned) as request:
            self.assertTrue(
                controller._owned_application_object("http_loadbalancers", "site-lan", "multi-cloud-networking")[
                    "owned"
                ]
            )
        ...
        with mock.patch.object(controller, "_xc_json", return_value={
            "metadata": {**owned["metadata"], "namespace": "system"}
        }):
            self.assertFalse(controller._owned_application_object(
    ...
    def test_application_namespace_is_selected_and_site_namespace_stays_system(self):
        ...
        for resource in ('xcsh_origin_pool" "home', 'xcsh_http_loadbalancer" "home'):
            ...
            self.assertIn('namespace   = var.application_namespace', block.split('\n}', 1)[0])
        self.assertIn('namespace = var.application_namespace', terraform.split('default_route_pools {', 1)[1])
        self.assertIn('namespace = "system"', terraform.split('origin_servers {', 1)[1].split('\n}', 1)[0])
