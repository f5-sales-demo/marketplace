...
except Exception as exc:  # GI can raise runtime-specific exceptions.
...
def walk(node, depth=0, inherited_pid=None) -> None:
    ...
    try:
        ...
        try:
            ...
            if candidate > 0:
                pid = candidate
        except Exception:  # Inaccessible AT-SPI nodes are isolated.
            pass
    ...
    except Exception:  # One bad subtree must not hide the desktop.
        return
