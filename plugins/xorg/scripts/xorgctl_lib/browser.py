...
def profile_lock_is_live(profile, lock, hostname=None, proc_root=pathlib.Path("/proc")):
    ...
    machine, separator, pid = target.rpartition("-")
    if (
        not separator
        or machine != (hostname or socket.gethostname())
        or not pid.isdigit()
    ):
        return True
...
def configure_chrome_fonts(profile):
    ...
    try:
        ...
        msg = "Chrome preferences are not valid JSON"
        raise Fault(msg) from exc
...
def browser(w, action, p):
    if action == "launch":
        mode = p.get("profile", "automation")
        profile = w.folder / "browser-profile"
        if mode == "default":
            profile = pathlib.Path.home() / ".config/google-chrome"
            if (profile / "SingletonLock").is_symlink() or (
                profile / "SingletonLock"
            ).exists():
                msg = "real Chrome profile is already owned; close it before launching"
                raise Fault(msg)
            flags = []
            ...
            msg = "profile must be automation or default"
            raise Fault(msg)
        lock = profile / "SingletonLock"
        if profile_lock_is_live(profile, lock):
            msg = (
                "browser profile already owned; use existing browser or another session"
            )
            raise Fault(msg)
        ...
        child = w.spawn(
            [
                ...
                *flags,
                ...
                "--start-maximized",
                p.get("url", "about:blank"),
            ]
        )
        return {
            "pid": child.pid,
            "profile": mode,
            "cdp_port": w.browser_port if mode == "automation" else None,
        }
    if not w.browser_port:
        msg = "semantic observation requires this session automation browser; launch it first"
        raise Fault(msg)
    request = {"port": w.browser_port, "action": action, "params": p}
    data = json.loads(
        w.command(
            [sys.executable, str(pathlib.Path(__file__).with_name("browser_probe.py"))],
    ...
    if action == "activate":
        ...
        wid = p.get("window")
        if wid:
            w.window("focus", {"window": wid})
        result = w.input.execute(
            {
                "steps": [
                    {
                        "action": "click",
        ...
        return {
            "target": data,
            "input": result,
            "verify": "take a fresh browser snapshot or screenshot",
        }
    return data
