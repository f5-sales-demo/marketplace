#!/usr/bin/env bash
# Phase 1: Structural validation — no gcloud CLI or session required.
...
test_expected_files_exist() {
  local files=(
    ...
    "hooks/hooks.json"
    "skills/gcloud-index/SKILL.md"
    "skills/gcloud-auth/SKILL.md"
...
test_skill_frontmatter() {
  for skill_dir in gcloud-index gcloud-auth; do
...
# T1.5 — gcloud-index is not user-invocable
test_index_not_user_invocable() {
  local skill="$PLUGIN_ROOT/skills/gcloud-index/SKILL.md"
  grep -q 'user-invocable: false' "$skill" || {
    echo "gcloud-index should be user-invocable: false"
    return 1
  }
}

# T1.6 — gcloud-auth is not user-invocable
test_auth_not_user_invocable() {
  local skill="$PLUGIN_ROOT/skills/gcloud-auth/SKILL.md"
  grep -q 'user-invocable: false' "$skill" || {
    echo "gcloud-auth should be user-invocable: false"
    return 1
  }
}
...
test_command_frontmatter() {
  for cmd in gcloud-login gcloud-status; do
...
test_T1_15_hook_references_setup_command() {
  ...
  if grep -Eq 'gcloud:setup|brew install|apt(-get)? install' "$hook"; then
