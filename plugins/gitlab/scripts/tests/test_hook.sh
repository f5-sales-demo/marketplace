#!/usr/bin/env bash
# Phase 2: SessionStart hook behavior — requires glab CLI.
...
# T2.1 — hook succeeds with no output when glab is installed
test_hook_succeeds_when_glab_installed() {
...
# T2.2 — hook warns when glab is not on PATH
test_hook_warns_when_glab_missing() {
  local cmd
  cmd=$(_get_hook_command)
  local tmp
  tmp=$(mktemp -d)
