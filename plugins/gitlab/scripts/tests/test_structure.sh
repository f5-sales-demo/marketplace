#!/usr/bin/env bash
# Phase 1: Structural validation — no glab CLI or session required.
...
test_plugin_json_valid() {
  ...
  name=$(jq -r '.name' "$pj")
  [ "$name" = "gitlab" ] || {
    echo "name=$name, expected gitlab"
    return 1
  }
...
test_marketplace_entry() {
  ...
  [ "$mp_name" = "gitlab" ] || {
    echo "gitlab entry missing from marketplace.json"
    return 1
  }

  local mp_ver
  mp_ver=$(jq -r '.plugins[] | select(.name == "gitlab") | .version' "$mj")
  local pj_ver
  ...
  local src
  src=$(jq -r '.plugins[] | select(.name == "gitlab") | .source' "$mj")
  [ "$src" = "./plugins/gitlab" ] || {
    echo "source=$src, expected ./plugins/gitlab"
    return 1
  }
}
...
test_expected_files_exist() {
  local files=(
    ...
    "commands/glab-login.md"
    "commands/glab-status.md"
  )
  for f in "${files[@]}"; do
    [ -f "$PLUGIN_ROOT/$f" ] || {
      echo "missing: $f"
...
test_skill_frontmatter() {
  for skill_dir in gitlab-index gitlab-auth; do
...
# T1.5 — gitlab-index is not user-invocable
test_index_not_user_invocable() {
  local skill="$PLUGIN_ROOT/skills/gitlab-index/SKILL.md"
  grep -q 'user-invocable: false' "$skill" || {
    echo "gitlab-index should be user-invocable: false"
    return 1
  }
}

# T1.6 — gitlab-auth is not user-invocable
test_auth_not_user_invocable() {
  local skill="$PLUGIN_ROOT/skills/gitlab-auth/SKILL.md"
  grep -q 'user-invocable: false' "$skill" || {
    echo "gitlab-auth should be user-invocable: false"
    return 1
  }
}
...
test_command_frontmatter() {
  for cmd in glab-login glab-status; do
...
test_T1_14_tool_factories_exist() {
  ...
  local missing=()
  for f in glab-setup.ts glab-issue-list.ts glab-issue-view.ts glab-search.ts; do
...
# T1.15 — hooks.json references /gitlab:setup, not brew
test_T1_15_hook_references_setup_command() {
  ...
  if ! grep -q "gitlab:setup" "$hook"; then
    echo "FAIL: hooks.json should reference /gitlab:setup"
    return 1
  fi
