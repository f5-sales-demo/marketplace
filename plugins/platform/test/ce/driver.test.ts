...
describe('HttpCeV2Driver capability contract', () => {
  it('checks out through only the capability-advertised v2 API', async () => {
    ...
    globalThis.fetch = (async (input: RequestInfo | URL) => {
      ...
      calls.push(url);
      if (url.endsWith('/api/web/capabilities/secure-mesh-site-v2')) return new Response(JSON.stringify({
        version: 'v2',
        endpoints: {
    ...
    const driver = new HttpCeV2Driver({ F5XC_API_URL: 'https://tenant.example.test', F5XC_API_TOKEN: 'fixture-api-credential' });
    const result = await driver.checkoutBootstrap({ namespace: 'system', siteName: 'ce-demo', nodeName: 'ce-1', expiresInSeconds: 60 }, false);
