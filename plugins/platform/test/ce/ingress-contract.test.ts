...
test('rejects altered release identity, publication receipt and asset binding before downloading schemas', async () => {
  for (const patch of [
    ...
    let assetReads = 0;
    await expect(
      VerifiedIngressContract.release(async (url) => {
...
test('propagates cancellation and rejects failed release requests', async () => {
  ...
  let requests = 0;
  await expect(
    VerifiedIngressContract.release(async () => {
      requests++;
      return response(metadata);
    }, control.signal),
  ).rejects.toThrow();
  expect(requests).toBe(0);
  await expect(VerifiedIngressContract.release(async () => new Response('', { status: 403 }))).rejects.toThrow();
});
