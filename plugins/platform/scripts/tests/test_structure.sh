...
test_expected_files_exist() {
  local files=(
    ...
    "src/index.ts"
    "src/tools/f5xc-ce-v2-site.ts"
    "src/tools/f5xc-ce-v2-bootstrap.ts"
    "src/tools/f5xc-ce-v2-status.ts"
  )
  for f in "${files[@]}"; do
    [ -f "$PLUGIN_ROOT/$f" ] || {
      echo "missing: $f"
...
test_platform_v5_ce_contract() {
  ...
  for legacy in \
    skills/api-operations/references/resources/cloud_infrastructure/azure_vnet_site.md \
    skills/api-operations/references/resources/ce_management/fleet_config.md \
    skills/api-operations/references/resources/ce_management/registration_token.md \
    skills/api-operations/references/workflows/register_site.md; do
    [[ ! -e "$PLUGIN_ROOT/$legacy" ]] || {
      echo "legacy CE route remains: $legacy"
      return 1
    }
  done
}
