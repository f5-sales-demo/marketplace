...
# T2.1 — hook succeeds silently when XCSH_API_TOKEN is set
test_hook_succeeds_when_token_set() {
  local cmd
  cmd=$(_get_hook_command)
  local out
  out=$(XCSH_API_TOKEN="test-token" bash -c "$cmd" 2>&1)
  [ -z "$out" ] || {
...
# T2.2 — hook outputs WARNING when XCSH_API_TOKEN is not set
test_hook_warns_when_token_unset() {
  local cmd
  cmd=$(_get_hook_command)
  local out
  out=$(
    unset XCSH_API_TOKEN
    bash -c "$cmd" 2>&1
  ) || true
