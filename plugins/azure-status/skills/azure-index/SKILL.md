...
- Auth setup -> invoke `azure-status:azure-auth` skill
- Account/subscription status -> delegate to `azure-status:cli-operator` agent:

  ```text
  Agent(
    subagent_type="azure-status:cli-operator",
...
- Resource queries -> delegate to `azure-status:cli-operator` agent with
