...
Run any `gh` (GitHub CLI) command. Pass arguments as an array without the `gh` prefix, e.g. `["pr", "list", "--json", "number,title"]`.

## Safety

- Arguments are passed argv-style to `gh` with no shell — shell metacharacters are inert.
- Read and write commands are passed through without a policy or environment-variable gate.
- Malformed inputs, control characters, cancellation, missing dependencies, and non-zero `gh` results remain errors.
