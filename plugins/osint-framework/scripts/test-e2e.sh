...
if [ -z "$LAYER_FILTER" ] || { [ "$LAYER_FILTER" = "--layer" ] && [ "${2:-}" = "2" ]; } || [ "$LAYER_FILTER" = "2" ]; then
  ...
  run_test "U08" "ipinfo.io returns IP JSON" \
    "curl -s 'https://ipinfo.io/192.0.2.72/json' 2>/dev/null" \
    "\"ip\"" 15

  run_test "U09" "whois IP returns NetName" \
    "whois 192.0.2.72 2>/dev/null" \
    "netname|orgname|org-name|NetName|OrgName" 30

  run_test "U10" "dig reverse DNS runs clean" \
    "dig -x 192.0.2.72 +short 2>/dev/null; echo 'reverse_done'" \
    "reverse_done" 15
  ...
  run_test "U12" "nmap service detection scanme.nmap.org" \
    "nmap -sV --top-ports 5 -T3 192.0.2.75 2>/dev/null" \
    "open" 120
...
if [ -z "$LAYER_FILTER" ] || { [ "$LAYER_FILTER" = "--layer" ] && [ "${2:-}" = "3" ]; } || [ "$LAYER_FILTER" = "3" ]; then
  ...
  run_test "T02a" "IP pipeline: geolocation lookup" \
    "curl -s 'https://ipinfo.io/192.0.2.75/json' 2>/dev/null | jq '{ip,city,org}' 2>/dev/null" \
    "\"ip\"" 15

  run_test "T02b" "IP pipeline: WHOIS ownership" \
    "whois 192.0.2.75 2>/dev/null | grep -iE 'orgname|org-name|netname'" \
    "." 30

  run_test "T02c" "IP pipeline: nmap service scan" \
    "nmap -sV --top-ports 10 -T3 192.0.2.75 2>/dev/null" \
    "open" 120

  run_test "T02d" "IP pipeline: searchsploit pivot from nmap" \
    "SERVICE=\$(nmap --top-ports 5 -T3 192.0.2.75 2>/dev/null | grep open | head -1 | awk '{print \$3}') && searchsploit \"\$SERVICE\" 2>/dev/null | head -3" \
    "." 60
  ...
  run_test "T03d" "Passive pipeline: ipinfo country" \
    "curl -s 'https://ipinfo.io/192.0.2.72/json' 2>/dev/null | jq -r '.country' 2>/dev/null" \
    "US" 15
