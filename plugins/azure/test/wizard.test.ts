...
describe('runSetupWizard — az installed, web auth', () => {
  ...
  it('reports version then proceeds to auth', async () => {
    const { pi } = buildMockPi({
      ...
      'account show': {
        stdout: JSON.stringify({ user: { name: 'history-f02f61d33aac@example.com' }, name: 'Test Subscription' }),
        stderr: '',
        code: 0,
      },
    });
  ...
  it('auto-detects service principal from environment', async () => {
    ...
    try {
      const { pi, calls } = buildMockPi({
        ...
        'account show': {
          stdout: JSON.stringify({ user: { name: 'history-bb9a18b3c2b3@example.com' }, name: 'SP Subscription' }),
          stderr: '',
          code: 0,
        },
      });
