...
export function assertObservationFresh(plan: AzureCePlan, current: AzureCeObservation, expectedFingerprint = plan.observationFingerprint): void {
  ...
  if (!safeHexEqual(expectedFingerprint, fingerprint)) {
    throw new Error(`Stale Azure CE plan: observations changed (expected ${expectedFingerprint}, current ${fingerprint})`);
  }
}
...
export function assertApplyAllowed(
  ...
  if (!safeHexEqual(request.planSha256, plan.planSha256)) throw new Error('The requested plan hash does not match the persisted plan');
  ...
  if (plan.actions.some((action) => action.kind === 'marketplace-terms-accept') && !request.hasUI && request.env.XCSH_AZURE_CE_ACCEPT_MARKETPLACE_TERMS !== '1') {
    throw new Error('Headless Marketplace terms acceptance requires XCSH_AZURE_CE_ACCEPT_MARKETPLACE_TERMS=1');
  }
}

export function resolveActionArgs(args: string[], planSha256: string, replacements: Record<string, string> = {}): string[] {
  return args.map((arg) => {
...
const CREATE_KINDS = new Set([
  'resource-group-create', 'vnet-create', 'subnet-create', 'nsg-create', 'nsg-rule-create',
  'public-ip-create', 'nic-create', 'vm-create', 'route-table-create', 'route-server-create',
...
export async function assertActionOwnership(plan: AzureCePlan, action: AzureCeAction, api: AzExecApi): Promise<void> {
  ...
  if (['route-association-update', 'brownfield-restore'].includes(action.kind)) {
    const allowed = plan.ownershipInventory.some((item) => item.action === 'modify-approved' && item.resourceId.toLowerCase() === action.resourceId?.toLowerCase());
    if (!allowed) throw new Error(`Brownfield resource is outside the approved allowlist: ${action.resourceId ?? '<missing>'}`);
    return;
  }
  if (!action.resourceId) {
    if (action.kind === 'route-create' && plan.intent.brownfield.routeChanges.length === 0) return;
    if (action.kind === 'vm-start' || action.kind === 'vm-stop' || action.kind === 'vm-deallocate' || action.kind === 'vm-resize' || action.kind === 'vm-delete') return;
  ...
  if (!CREATE_KINDS.has(action.kind) && action.kind !== 'resource-delete' && !OWNED_MUTATION_KINDS.has(action.kind)) return;
  ...
  if (result.exitCode !== 0) {
    if (CREATE_KINDS.has(action.kind) && /not found|could not be found|resourcegroupnotfound/i.test(result.stderr)) return;
    if (action.kind === 'resource-delete' && /not found|could not be found|resourcegroupnotfound/i.test(result.stderr)) return;
    throw new Error(`Unable to verify ownership for ${action.resourceId}`);
  }
  let raw: Record<string, unknown>;
  try { raw = JSON.parse(result.stdout) as Record<string, unknown>; } catch { throw new Error(`Ownership response was invalid for ${action.resourceId}`); }
  const observedId = String(raw.id ?? '').toLowerCase();
  if (observedId && observedId !== action.resourceId.toLowerCase()) throw new Error(`Azure substituted a different resource ID for ${action.resourceId}`);
  ...
  if (CREATE_KINDS.has(action.kind) && tags['xcsh-plan-sha256'] !== plan.planSha256) throw new Error(`Existing resource belongs to a different Azure CE plan: ${action.resourceId}`);
}
