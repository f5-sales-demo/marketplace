...
export const AZURE_CE_RESEARCH_GATE = [
  ...
  'Execute this workflow directly. Do not delegate any research or discovery step to task, agents, or generic helpers; delegated workers cannot satisfy the required Azure tool evidence.',
  'Before any recommendation or azure_ce_plan call, use web_search to retrieve and read the dedicated f5xc-ce-automation-policy/v2 contract from f5-sales-demo.github.io, the current official F5 Secure Mesh Site v2 Azure deployment guide from docs.cloud.f5.com, and Microsoft Marketplace image and subscription-aware VM SKU guidance from learn.microsoft.com.',
  ...
  'For an initial Terraform deployment only, one idempotent terraform apply accepts an unaccepted exact discovered F5 XC Marketplace plan through the Marketplace Ordering current agreement GET plus idempotent PUT before any foundation resource is created. It requires no human acceptance, separate terms authorization, CLI command, manual state action, import, taint, rediscovery, or replan. Do not accept arbitrary offers or claim this applies to native, replacement, or repair workflows.',
